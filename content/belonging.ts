...
export const BELONGING_SECTIONS: BelongingSection[] = [
  ...
  {
    slug: "starting-a-club-penn",
    title: "If you want to start a club here",
    intro: "The process for starting a club at Penn changed significantly this academic year, so anything you've heard secondhand from an older student may already be out of date.",
    universityIds: ["penn"],
    body: [
      "Until the 2025-26 academic year, new-group registration at Penn was rolling -- submit whenever you had an idea, get reviewed roughly as you went. That's gone now. New groups have to apply by the end of October, all applications get reviewed together afterward, and decisions aren't final until sometime in December. The Office of Student Affairs has said the goal was efficiency, but the first year under the new system reportedly saw far more applications than expected, and the review period ran closer to two months than the one originally planned -- worth knowing so you don't panic if a decision takes a while.",
      "Penn also recently retired its old two-tier system, where basic registration and a separate, harder \"SAC Recognition\" used to be different things with different applications. Now there's just one registration status through PennClubs, and it's what actually unlocks space reservations, a Penn email account, and activities-fair access. SAC (the Student Activities Council) still exists, but its role narrowed to just funding decisions for groups that want it, not gatekeeping registration itself.",
      "If your idea fits certain categories, funding still routes through a specific governing board rather than SAC directly: performing arts groups go through the Performing Arts Council, religious and spiritual groups through PRISM, cultural groups through one of a few coalitions (APSC, the Latinx Coalition, UMOJA, or UMC), and competitive sports clubs through the Sports Club Council instead of SAC entirely. Worth figuring out which lane applies before you start the paperwork, since each has its own process.",
      "The application itself asks for a mission statement, a proposed leadership structure, a membership roster, and a constitution -- and you'll specifically need to explain what unique need your group meets that isn't already covered by one of Penn's roughly 450 active clubs. Checking the PennClubs directory closely before you apply isn't just good practice here, it's functionally part of the application.",
      "One more thing worth knowing going in: Penn's club culture, especially in finance- and consulting-adjacent spaces, has a real reputation for being unusually competitive -- some clubs have reported admitting well under 10% of applicants some semesters. Student government has pushed back on this over the years with rules like a cap on interview rounds and a ban on collecting resumes from freshmen, and there's been at least one student-led effort to build a public club-review resource in response. If you go through recruitment and don't get in somewhere you wanted, that's a genuinely common experience here, not a signal about you.",
    ],
  },
];
